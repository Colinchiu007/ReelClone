...
describe('订单 API（order-service）', () => {
  ...
  describe('GET /packages（公开）', () => {
    test('套餐列表返回数组', async () => {
      const list =
        await orderClient.get<Array<{ id: string; name: string; status: string }>>('/packages')
    ...
    test('不存在的套餐返回 404', async () => {
      await expect(orderClient.get('/packages/non_existent_id')).rejects.toThrow(ApiError)
    })
  })
  ...
  describe('POST /orders（创建订单）', () => {
    ...
    test('创建订单幂等性：相同 idempotencyKey 返回同一订单', async () => {
      ...
      const r1 = await orderClient.post<{ orderId: string; orderNo: string }>('/orders', payload1)
      const r2 = await orderClient.post<{ orderId: string; orderNo: string }>('/orders', payload2)
    ...
    test('无效套餐 ID 应返回 404', async () => {
      await expect(
        orderClient.post('/orders', buildCreateOrderPayload('non_existent_pkg')),
  ...
  describe('POST /orders/:id/cancel（取消订单）', () => {
    test('取消 PENDING 订单成功', async () => {
      ...
      // 查询确认状态
      const order = await orderClient.get<{ status: string }>(`/orders/${created.orderId}`)
    ...
    test('取消已取消订单应失败', async () => {
      ...
      // 再次取消应失败
      await expect(orderClient.post(`/orders/${created.orderId}/cancel`)).rejects.toThrow(ApiError)
    })
  })
  ...
  describe('POST /webhooks/wechat-pay（支付回调幂等性）', () => {
    ...
    test('回调对应订单不存在时返回 SUCCESS（避免微信重试）', async () => {
      const callback = buildWechatPayCallbackPayload('RC_NOT_EXIST_ORDER_000000000001')
      const result = await orderClient.post<{ code: string }>(
        '/webhooks/wechat-pay',
        callback.body,
        { headers: callback.headers },
  ...
  describe('订单所有权校验', () => {
    test('不可访问他人订单', async () => {
      ...
      const loginA = await userA.wechatLogin(buildWechatLoginPayload({ nickname: '用户A' }).code)
      ...
      await expect(orderClientB.get(`/orders/${created.orderId}`)).rejects.toThrow(ApiError)

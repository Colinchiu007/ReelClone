...
describe('通知 API（notification-service）', () => {
  ...
  describe('GET /notifications/unread-count', () => {
    test('查询未读数量', async () => {
      const result = await notificationClient.get<{ count: number }>(
  ...
  describe('业务事件触发通知', () => {
    test('赠送积分后应产生通知', async () => {
      const before = await notificationClient.get<{ count: number }>(
        '/notifications/unread-count',
      );
      ...
      await billingClient.post(
        '/points/grant',
        buildGrantPointsPayload(userId, 10, 'order_notify_test', 'pkg_test_starter'),
        { internal: true },
      );
      ...
      const after = await notificationClient.get<{ count: number }>(
        '/notifications/unread-count',
      );
      // 未读数应 >= 之前（可能产生新通知）
  ...
  describe('POST /notifications/read-all', () => {
    test('全部标记已读后未读数为 0', async () => {
      const result = await notificationClient.post<{ affected: number }>(
      ...
      const unread = await notificationClient.get<{ count: number }>(
        '/notifications/unread-count',
      );
  ...
  describe('鉴权', () => {
    test('未授权访问应被拒绝', async () => {
      ...
      await expect(
        anon.get('/notifications', undefined, { raw: true }),

...
describe('认证 API（auth-service）', () => {
  ...
  describe('GET /auth/health', () => {
    test('健康检查返回 ok', async () => {
      ...
      const result = await client.get<{ status: string; service: string }>(
        '/auth/health',
        undefined,
        { raw: true },
      );
  ...
  describe('POST /auth/wechat-login', () => {
    test('Mock 模式登录成功，返回 token 与 user', async () => {
      ...
      const result = await client.wechatLogin(
        payload.code,
        payload.nickname,
        payload.avatarUrl,
      );
    ...
    test('空 code 应被校验拦截', async () => {
      const c = createClient('auth');
      await expect(
        c.post('/auth/wechat-login', { code: '' }, { raw: true }),
  ...
  describe('POST /auth/refresh-token', () => {
    test('用 refreshToken 换取新的 token 对', async () => {
      const c = createClient('auth');
      const payload = buildWechatLoginPayload();
      const loginResult = await c.wechatLogin(
        payload.code,
        payload.nickname,
        payload.avatarUrl,
      );
  ...
  describe('POST /auth/logout', () => {
    test('登出后将 Token 加入黑名单，后续请求应被拒绝', async () => {
      ...
      await expect(userClient.get('/users/me', undefined, { raw: true })).rejects.toThrow(
        ApiError,
      );

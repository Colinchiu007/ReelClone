...
describe('用户 API（user-service）', () => {
  ...
  describe('GET /users/me', () => {
    ...
    test('未授权访问应被拒绝', async () => {
      ...
      await expect(anon.get('/users/me', undefined, { raw: true })).rejects.toThrow(
        ApiError,
      );
    });
  });
  ...
  describe('PUT /users/me', () => {
    ...
    test('空昵称应被校验拦截', async () => {
      await expect(
        userClient.put('/users/me', { nickname: '' }),
  ...
  describe('POST /sms/send（Mock 模式）', () => {
    test('发送短信验证码', async () => {
      ...
      await expect(userClient.post('/sms/send', { mobile, purpose: 'bind_mobile' })).resolves.toBeDefined();
    });

    test('无效手机号应被校验拦截', async () => {
      await expect(
        userClient.post('/sms/send', { mobile: '123', purpose: 'bind_mobile' }),
      ).rejects.toThrow(ApiError);
    });
  });
  ...
  describe('POST /users/bind-mobile', () => {
    test('绑定手机号（Mock 模式验证码任意）', async () => {
      mobile = randomMobile();
      // 先发送验证码
      await userClient.post('/sms/send', { mobile, purpose: 'bind_mobile' });

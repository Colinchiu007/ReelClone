...
describe('用户路径2: 素材上传 → 生成视频 → 下载作品', () => {
  ...
  test('5. 轮询等待作品完成（Mock 模式立即完成）', async () => {
    ...
    const work = await poll({
      fn: () => workbenchClient.get<{ id: string; status: string }>('/works', {
        page: 1,
        pageSize: 50,
      }),
    ...
    expect(work.list.some((w) => w.id === workId)).toBe(true);

    // 轮询作品详情直到状态稳定（COMPLETED 或 FAILED 都视为终态）
    const detail = await poll({
      fn: () => workbenchClient.get<{ id: string; status: string; resultUrl?: string }>(
        `/works/${workId}`,
      ),
  ...
  test('7. 删除资产（DELETE /assets/:id）', async () => {
    ...
    // 删除后列表不应再包含该资产
    const list = await assetClient.get<{ list: Array<{ id: string }>; total: number }>(
      '/assets',
      { page: 1, pageSize: 50 },
    );

...
describe('用户路径3: 对标解析 → 基于拆解生成新视频', () => {
  ...
  test('2. 查看解析历史包含刚提交的任务', async () => {
    // 守卫：前置 test 失败时 benchmarkId 为 undefined
  ...
  test('3. 轮询等待解析完成（Mock 模式立即完成）', async () => {
    const detail = await poll({
      ...
      predicate: (b) => b.status === 'COMPLETED' || b.status === 'FAILED' || b.status === 'SUCCESS',
      timeout: 20000,
  ...
  test('5. 基于解析结果提交新视频生成任务', async () => {
    ...
    const promptSource =
      ...
      (detail.result?.keywords?.length ? detail.result.keywords.join('，') : '对标解析参考内容')

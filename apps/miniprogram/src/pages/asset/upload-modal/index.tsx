...
import { BaseModal, MediaUploader, IndustryPicker } from '@/components'
...
export default function UploadModal({
  ...
  return (
    <BaseModal
      ...
      onClose={handleClose}
      scrollable
      bodyClassName="upload-modal__body"
      footerClassName="upload-modal__footer"
      footer={
        <>
          {busy && (
          ...
          <View
            className={`upload-modal__submit ${busy ? 'upload-modal__submit--disabled' : ''}`}
            onClick={handleSubmit}
          >
            <Text>{submitting ? '提交中...' : uploading ? '上传中...' : '确认上传'}</Text>
          </View>
        </>
      }
    >
      ...
      <View className="upload-modal__section">
        <Text className="upload-modal__label">选择文件（最多 {MAX_FILE_COUNT} 个）</Text>
        <MediaUploader
          key={fileType}
          type={fileType}
        ...
        />
        <Text className="upload-modal__hint">大小限制：图片 10MB / 视频 100MB / 音频 50MB</Text>
      </View>
      ...
      {/* 标签输入 */}
      <View className="upload-modal__section">
        <Text className="upload-modal__label">标签（可选，最多 {MAX_TAGS} 个）</Text>
          ...
          <View className="upload-modal__tag-list">
            {tags.map((tag, idx) => (
              <View key={tag + idx} className="upload-modal__tag">
                ...
                <Text className="upload-modal__tag-del" onClick={() => handleRemoveTag(idx)}>
                  ×
                </Text>
              </View>
            ))}

...
import {
  createBenchmark,
  listBenchmarks,
} from '@/services/api/benchmark.api';
...
export default function Index() {
  ...
  const handleHistoryClick = (item: Benchmark) => {
    if (item.status === 'COMPLETED') {
      Taro.showToast({ title: '详情页即将上线', icon: 'none' });
    } else {
      Taro.showToast({ title: '解析进行中，请稍后', icon: 'none' });
    }
  };
  ...
  return (
    <View className='benchmark'>
      ...
      <View className='benchmark__input-section'>
        ...
        <Text className='benchmark__hint'>
          支持抖音、小红书、B站、快手、微博等主流平台视频链接
        </Text>
      </View>
      ...
      <View className='benchmark__history'>
        <View className='benchmark__history-header'>
          ...
          <Text className='benchmark__history-refresh' onClick={loadHistory}>刷新</Text>
        </View>

...
import { listFavorites, unfavoriteTemplate } from '@/services/api/template.api'
  ...
  return (
    <View className="my-tpl">
      ...
      <View className="my-tpl__list">
        {tab === 'history' ? (
          <EmptyState title="历史功能即将上线" description="浏览过的模板将在这里展示" icon="🕒" />
        ) : loading && list.length === 0 ? (
          <LoadingState />
        ) : error && list.length === 0 ? (
          <ErrorState onRetry={() => fetchList(1, true)} />
        ) : list.length === 0 ? (
          <EmptyState title="还没有收藏任何模板" description="去灵感广场发现更多灵感吧" icon="📭" />
        ) : (

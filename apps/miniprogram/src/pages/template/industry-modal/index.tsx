...
import { BaseModal, IndustryPicker } from '@/components'
...
export default function IndustryModal({
  ...
  return (
    <BaseModal
      visible={visible}
      title="选择你感兴趣的行业（1-3个）"
      closable={closable}
      busy={saving}
      onClose={onClose}
      footer={
        <View
          className={`industry-modal__btn ${canSave ? '' : 'industry-modal__btn--disabled'}`}
          onClick={handleSave}
        >
    ...
    >
      <IndustryPicker value={industries} onChange={setIndustries} max={3} min={1} />
    </BaseModal>
  )
}

...
export default function GalleryPage() {
  ...
  /** 收藏切换：乐观更新 + 失败回滚 */
  const handleFavorite = useCallback(async (id: string, next: boolean) => {
    const snapshot = list;
    setList((prev) =>
      ...
      <View className='gallery__header'>
        ...
        <ScrollView className='gallery__platforms' scrollX showScrollbar={false}>
          {PLATFORM_TABS.map((tab) => (
            <View
              key={tab.key || 'all'}
              className={`gallery__platform ${
                platform === tab.key ? 'gallery__platform--on' : ''
              }`}
          ...
          {SORT_TABS.map((tab) => (
            <View
              ...
              className={`gallery__sort ${
                sortBy === tab.key ? 'gallery__sort--on' : ''
              }`}

/**
 * 图生视频-首尾帧 工作台
 * 对应 FR2_视频生成_03_图生视频首尾帧
 *
 * - 首帧上传 + 尾帧上传（2 个 MediaUploader）
 * - 参数：模型 / 分辨率 / 宽高比 / 时长
 * - 提示词：可选
 */
...
export default function VideoImageFirstLastWorkbench() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (lastFrame.length === 0) {
      Taro.showToast({ title: '请上传尾帧图片', icon: 'none' });
      return;
    }
  ...
  }, [firstFrame, lastFrame, prompt, balance, estimatedPoints, model, resolution, aspectRatio, duration, consume]);

  const currentModelLabel = MODELS.find((m) => m.value === model)?.label ?? '';

  return (
    <View className='video-fl-wb page-wrap'>
      <View className='page-wrap__header'>
        <Text className='page-wrap__title'>图生视频 · 首尾帧</Text>
        <View className='page-wrap__credits'>
          <CreditBadge amount={balance} size='sm' />
        </View>
      </View>

      <View className='video-fl-wb__body'>
        {/* 首帧上传 */}
        ...
        {/* 尾帧上传 */}
        <View className='page-wrap__section'>
          <Text className='page-wrap__label'>尾帧图片 *</Text>
          <MediaUploader
            type='image'
            maxCount={1}
            value={lastFrame}
        ...
        <View className='page-wrap__section'>
          ...
          <View
            className='video-fl-wb__select'
            onClick={() => setShowModelSheet(true)}
          >
            <Text>{currentModelLabel}</Text>
            <Text className='video-fl-wb__select-arrow'>›</Text>
          </View>
        </View>
        ...
        <View className='page-wrap__section'>
          ...
          <View className='page-wrap__row'>
            {RESOLUTIONS.map((r) => (
              <View
                key={r}
                className={`page-wrap__chip ${
                  resolution === r ? 'page-wrap__chip--on' : ''
                }`}
          ...
          <View className='page-wrap__row'>
            {ASPECT_RATIOS.map((r) => (
              <View
                key={r}
                className={`page-wrap__chip ${
                  aspectRatio === r ? 'page-wrap__chip--on' : ''
                }`}
        ...
        <View className='page-wrap__section'>
          ...
          <View className='video-fl-wb__duration'>
            {DURATIONS.map((d) => (
              <View
                key={d}
                className={`video-fl-wb__duration-item ${
                  duration === d ? 'video-fl-wb__duration-item--on' : ''
                }`}
        ...
        {/* 预计消耗积分 */}
        <View className='video-fl-wb__estimate'>
          <Text className='video-fl-wb__estimate-label'>预计消耗</Text>
          <Text className='video-fl-wb__estimate-value'>
            {estimatedPoints} 积分
          </Text>
        </View>
      ...
      <View className='video-fl-wb__footer'>
        <View
          className={`page-wrap__btn ${
            submitting || firstFrame.length === 0 || lastFrame.length === 0
              ? 'page-wrap__btn--disabled'
              : ''
          }`}
          ...
          >
            ...
            {MODELS.map((m) => (
              ...
              >
                <Text>{m.label}</Text>
                {model === m.value ? (
                  <Text className='action-sheet__check'>✓</Text>
                ) : null}
              </View>
            ))}
            <View

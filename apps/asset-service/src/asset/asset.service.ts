...
import { OSSService, STSService, generateAssetKey, type STSToken } from '@reelclone/oss'
...
export class AssetService {
   ...
   */
  async createUploadToken(userId: string, dto: UploadTokenDto): Promise<UploadTokenResult> {
    ...
    const token = await this.stsService.generateUploadToken(userId, resourcePrefix, 3600, key)
    return {
      key,
  ...
  /**
   * 资产列表（分页 + 筛选，仅当前用户的可用资产）
   * 包含 ACTIVE 和 PENDING_REVIEW 状态（新建资产待审核时仍可见）
   */
  async findAll(userId: string, dto: ListAssetsDto): Promise<PaginatedResult<Asset>> {
  ...
  async create(userId: string, dto: CreateAssetDto): Promise<Asset> {
    ...
    if (!dto.ossKey.startsWith(expectedPrefix)) {
      this.logger.warn(`ossKey prefix mismatch: expected ${expectedPrefix}, got ${dto.ossKey}`)
      throw new BusinessException(ErrorCode.VALIDATION_ERROR, 'ossKey 前缀不匹配，无权创建资产')
    }
    ...
    if (avatarGroup) {
      await this.avatarGroupRepo.increment({ id: avatarGroup.id }, 'assetCount', 1)
    }
  ...
  /**
   * 资产详情（校验所有权，允许 ACTIVE 和 PENDING_REVIEW）
   */
  ...
  async delete(userId: string, id: string): Promise<{ success: boolean }> {
    ...
    if (!ossDeleted) {
      this.logger.warn(`OSS 文件删除失败 assetId=${id} ossKey=${asset.ossKey}，继续删除数据库记录`)
    }
   ...
   * 校验真人形象组归属与状态
   */
  private async findOwnedAvatarGroup(userId: string, avatarGroupId: string): Promise<AvatarGroup> {
   ...
   * 递减形象组素材数量（兜底不低于 0）
   */
  private async decrementAvatarGroupAssetCount(avatarGroupId: string): Promise<void> {
    const group = await this.avatarGroupRepo.findOne({
    ...
    await this.avatarGroupRepo.update({ id: avatarGroupId }, { assetCount: next })
  }
}

...
export class AvatarGroupService {
  ...
  async findAll(
    userId: string,
    dto: ListAvatarGroupsDto,
  ): Promise<PaginatedResult<AvatarGroup>> {
  ...
  async update(
    ...
    if (dto.authorizationStatus !== undefined) {
      group.authorizationStatus = dto.authorizationStatus;
    }
  ...
  async delete(userId: string, id: string): Promise<{ success: boolean }> {
    ...
    for (const asset of assets) {
      ...
      if (!ossDeleted) {
        this.logger.warn(
          `OSS 文件删除失败 assetId=${asset.id} ossKey=${asset.ossKey}`,
        );
      }
    }
    if (assets.length > 0) {
    ...
    this.logger.log(
      `User ${userId} deleted avatar group ${id} (cascaded ${assets.length} assets)`,
    );
    return { success: true };
  }

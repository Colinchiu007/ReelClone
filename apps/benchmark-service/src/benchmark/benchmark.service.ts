...
export class BenchmarkService {
  ...
  async create(
    ...
    if (cached) {
      ...
      this.logger.log(
        `幂等命中，返回已有 benchmark benchmarkId=${record.benchmarkId}`,
      );
      return record;
    }
  ...
  async findAll(
    userId: string,
    dto: ListBenchmarksDto,
  ): Promise<PaginatedBenchmarks> {
    const repo: Repository<Benchmark> =
      this.benchmarkDataSource.getRepository(Benchmark);
  ...
  private async compensateRelease(
    ...
    try {
      ...
      if (!freezeId) {
        this.logger.warn(
          `未找到 freezeId，跳过释放积分 benchmarkId=${benchmarkId}`,
        );
        return;
      }

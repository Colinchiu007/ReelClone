...
describe('BenchmarkService', () => {
  ...
  describe('create', () => {
    ...
    it('不支持的平台应抛出异常', async () => {
      ...
      await expect(service.create('user-001', badDto)).rejects.toThrow(
        BusinessException,
      );
    ...
    it('冻结积分失败时应更新状态为 FAILED 并抛出', async () => {
      billingClient.freeze.mockRejectedValue(
        BusinessException.insufficientCredits('积分不足'),
      );

      await expect(service.create('user-001', dto)).rejects.toThrow(
        BusinessException,
      );
    ...
    it('各平台 URL 应正确识别', async () => {
      ...
      for (const tc of testCases) {
        ...
        expect(repo.create).toHaveBeenCalledWith(
          expect.objectContaining({ platform: tc.expected }),
        );
      }
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('无权访问他人 benchmark 时抛 FORBIDDEN', async () => {
      ...
      await expect(service.findOne('user-001', 'b1')).rejects.toThrow(
        BusinessException,
      );
    });
  });
  ...
  describe('cancel', () => {
    ...
    it('非 Mock 模式应调用 Temporal cancelWorkflow', async () => {
      ...
      expect(temporalAdapter.cancelWorkflow).toHaveBeenCalledWith(
        'benchmark-b1',
        '用户主动取消',
      );
    });
    ...
    it('COMPLETED 状态不可取消', async () => {
      ...
      await expect(service.cancel('user-001', 'b1')).rejects.toThrow(
        BusinessException,
      );

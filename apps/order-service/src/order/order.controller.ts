...
export class OrderController {
  ...
  async list(@CurrentUser('userId') userId: string, @Query() dto: ListOrdersDto) {
   ...
   */
  @Get(':id')
  @ApiOperation({ summary: '订单详情（校验所有权）' })
  async detail(@CurrentUser('userId') userId: string, @Param('id') id: string) {
   ...
   */
  @Post(':id/cancel')
  @ApiOperation({ summary: '取消订单（仅 PENDING 状态可取消）' })

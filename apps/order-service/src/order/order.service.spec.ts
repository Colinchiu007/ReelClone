 ...
 *  - handleCallback：成功 / 订单不存在 / 幂等（已 PAID）/ 非 SUCCESS 状态
 */
...
describe('OrderService', () => {
  ...
  describe('createOrder', () => {
    ...
    it('套餐不存在时抛 NOT_FOUND', async () => {
      ...
      await expect(service.createOrder('user-001', dto)).rejects.toThrow(
        BusinessException,
      );
  ...
  describe('findOne', () => {
    ...
    it('订单不存在时抛 NOT_FOUND', async () => {
      ...
      await expect(service.findOne('user-001', 'nope')).rejects.toThrow(
        BusinessException,
      );
    });
    ...
    it('订单存在但不属于当前用户时也抛 NOT_FOUND（不暴露存在性）', async () => {
      ...
      await expect(service.findOne('user-001', 'o1')).rejects.toThrow(
        BusinessException,
      );
    });
  });
  ...
  describe('cancel', () => {
    ...
    it('PAID 状态订单不可取消', async () => {
      ...
      await expect(service.cancel('user-001', 'o1')).rejects.toThrow(
        BusinessException,
      );
      expect(orderRepo.save).not.toHaveBeenCalled();
    });
  ...
  describe('handleCallback', () => {
    it('成功处理回调：更新订单为 PAID 并调用 billing grant', async () => {
      ...
      wechatPay.verifyCallback.mockResolvedValue(true);
      wechatPay.decryptResource.mockResolvedValue({
    ...
    it('订单已 PAID 时幂等返回（不重复处理）', async () => {
      ...
      const result = await service.handleCallback({ body: { resource: {} } });
    ...
    it('签名校验失败时抛异常', async () => {
      wechatPay.verifyCallback.mockResolvedValue(false);

      await expect(
        service.handleCallback({ body: { resource: {} } }),

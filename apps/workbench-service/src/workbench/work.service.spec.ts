...
describe('WorkService', () => {
  ...
  let templateClient: { publishTemplate: jest.Mock }
  ...
  beforeEach(() => {
    ...
    templateClient = {
      publishTemplate: jest.fn().mockResolvedValue({ id: 'tmpl-1' }),
    }

    service = new WorkService(dataSource, templateClient as never)
  })
  ...
  describe('findAll', () => {
    it('分页返回作品列表', async () => {
      const qb = {
        ...
        getManyAndCount: jest.fn().mockResolvedValue([[{ id: 'work-1' } as Work], 1]),
      }
  ...
  describe('findOne', () => {
    ...
    it('无权限访问时抛异常', async () => {
      ...
      await expect(service.findOne('user-1', 'work-1')).rejects.toThrow(BusinessException)
    })

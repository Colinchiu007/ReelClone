...
export class GenerationService {
  ...
  async cancel(userId: string, taskId: string): Promise<void> {
    ...
    if (freezeId) {
      ...
      try {
        ...
        this.logger.error(
          `释放积分失败: taskId=${taskId} - ${(err as Error).message}`,
        );
      }
    }
  }
  ...
  private async startWorkflow(
    ...
    if (this.isMockMode()) {
      ...
      this.logger.log(
        `[Mock] 模拟工作流已启动 workId=${work.id} taskId=${task.id}`,
      );
      return;
    }
    ...
    if (!isVideoType(dto.generationType)) {
      ...
      this.logger.log(
        `非视频类型，直接标记运行中 workId=${work.id} taskId=${task.id}`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `工作流已启动 workId=${work.id} workflowId=${workflowId}`,
      );
    } catch (err) {
      // 工作流启动失败：标记任务失败 + 释放积分
      this.logger.error(
        `启动工作流失败 workId=${work.id}: ${(err as Error).message}`,
      );
      await taskRepo.update(task.id, {
        status: GenerationTaskStatus.FAILED,
      ...
      if (freezeId) {
        ...
        try {
          ...
          this.logger.error(
            `释放积分失败 workId=${work.id}: ${(releaseErr as Error).message}`,
          );
        }
      }
      throw BusinessException.taskFailed(
        `工作流启动失败: ${(err as Error).message}`,
      );
    }
  }

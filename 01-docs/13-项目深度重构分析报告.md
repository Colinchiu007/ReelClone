...
| 工作项                                                         | 优先级 | 成本 | 验收                                                                        |
| -------------------------------------------------------------- | -----: | ---: | --------------------------------------------------------------------------- |
| 微信登录 fail closed；Access/Refresh 与统一撤权协议            |     P0 |    M | 缺凭证拒绝 ready；全部服务拒绝 Refresh Bearer、旧权限和吊销 session         |
| 支付 real mode fail closed，完成请求/回调验签                  |     P0 |    M | 签名/重放通过；appid、mchid、订单号、transaction 唯一性、金额、币种全量匹配 |
| benchmark freeze/settle/release 独立 key + durable reservation |     P0 |    M | 成功 settle、失败 release、补偿可重放；无 Redis-only 资金状态               |
| admin grant 使用 adjustment UUID；订单 paid-grant outbox       |     P0 |    M | 重放不重复加分，`user_packages.order_id` 唯一                               |
| mock billing 隔离；修复 completed + OPEN                       |     P0 |  S-M | invariant test 证明所有终态无过期 OPEN reservation                          |
| CI 执行真实 Nx/Taro/Vite build 和正确 Docker matrix            |     P0 |    M | PR 产生可启动制品，缺 Dockerfile/产物即失败                                 |
| fresh DB/Temporal bootstrap contract                           |     P0 |    M | 密码一致、namespace 自动创建、migration 失败不可跳过、worker 真正 ready     |
| 统一公开 `/livez` `/readyz`                                    |     P0 |  S-M | dependency down 返回 503；Compose 能从空环境收敛                            |
| 外部 URL/asset ownership policy                                |     P0 |    M | SSRF/private IP/redirect/跨用户 asset 测试通过                              |

### Phase 1：一致性内核（2-5 周）

1. 新建 `CreditOperation`，迁移 legacy ledger 的 GRANT/REWARD/CONSUME/FREEZE/RELEASE；模板奖励按 ordinal 枚举缺口，不按 count 猜测。
2. 新建 `GenerationExecution`、start outbox、terminal decision 和 reconciler。
3. outbox 增加 lease/backoff/dead-letter/replay，并接 backlog/age 告警。
4. capability registry 统一 quote、validation、UI 和 Provider mapping。
5. runtime profile 和 typed configuration 在启动阶段 fail closed。
6. 对历史数据做只读 inventory；无法证明关联的记录进入人工 reconciliation case，禁止按金额/描述猜测。

### Phase 2：模块边界（5-8 周）

1. 先拆 `foundation` 与 `platform-data`，消除 `common -> database`。
2. 拆 Temporal contracts/workflows/adapters，移除全局 dependency singleton。

...
已实现。`@reelclone/capability` 包统一了 8 种生成类型的 Provider 路由、积分定价（FixedPoints + MatrixPoints）、参数校验、模型/UI 配置和 work type 映射。后端 3 处硬编码（shared.ts / points-calculator / create.dto）和前端 7 个页面已迁移到 registry。31 个单元测试全通过。

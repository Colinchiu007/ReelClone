...
### 1.1 总体判断（基线：2026-08-01 审计；更新：2026-08-03 重构进展）

| 维度       | 基线 | 当前     | 核心变化                                                                                                                                                                                      |
| ---------- | ---- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 业务架构   | 黄   | **黄绿** | D1 消除 common→database；D2 Temporal 契约分离；D3 GenerationService 872→132 行；D5 按需 DB 初始化                                                                                             |
| 生成链路   | 红   | **黄**   | C1 GenerationExecution 覆盖 create/start/cancel/finalize；C5 ReconcilerWorkflow 定期扫描不确定状态；P0-5 Mock 隔离                                                                            |
| 计费一致性 | 红黄 | **黄**   | C6 outbox 增加 lease/backoff/dead-letter/replay + 告警；C9 历史数据只读 inventory；✅ benchmark 已迁移到 V2 CreditReservation（freeze/reservationMode=true + 成功时 settle + 失败时 release） |
| 安全       | 红   | **黄**   | P0-1 JWT 统一策略 + 撤权；P0-2 支付签名修复；P0-6 ossKey 前缀校验 + validateRedirect                                                                                                          |
| 测试       | 黄   | **黄绿** | capability 44T + workbench 33T + capability 评审修复 12T + benchmark V2 验证 11T；关键路径覆盖改善                                                                                            |
| CI/CD      | 红   | **红黄** | P0-5 Docker matrix 修正 + 小程序构建门禁；但镜像 digest 发布、E2E、migration contract 仍缺                                                                                                    |
| 可观测性   | 黄红 | **黄红** | 无显著变化；统一 trace context 和 OTel 待 Phase 3 推进                                                                                                                                        |
| 文档一致性 | 红   | **红黄** | 重构报告已同步 Phase 0-2 全部实现记录；旧架构文档（K8s/Formance/RabbitMQ）仍超前于实时代码                                                                                                    |
...
> **Benchmark 迁移已完成（2026-08-03）：** benchmark 计费已从 legacy LedgerService（`reservationMode=undefined`）迁移到 V2 CreditReservationService（`reservationMode=true`）。修复了 3 个严重缺陷：(1) 成功时从未 settle 导致积分永久冻结；(2) freeze/settle/release 共用幂等键导致 release 被 FREEZE 结果短路；(3) freezeId 只存在 Redis 无持久化。Temporal workflow 已集成 settle（成功分支）和 release（catch 块）。灰度方案见 `01-docs/14-benchmark-v2-billing-灰度方案.md`。剩余 legacy 双写（order/template/admin）仍在 P0-3 范围内。
...
> **已完成（2026-08-03）**：`@reelclone/common` 完全不依赖任何 `@reelclone/*` 包。分两步完成：
>
> 1. 早期会话将 `audit-log` 和 `config-store` 移至 `@reelclone/platform-data`，消除了 `common → database` 依赖。
> 2. 本次将 `RedisBridgeModule`（`common → database` + `common → observability` 的最后违规点）移至 `@reelclone/platform-data`，从 `common` 中删除 `redis-bridge.module.ts`、移除 `@reelclone/database` peer dependency、移除 index.ts 中 `platform-data` re-export。10 个微服务的 import 路径已全部更新为 `from '@reelclone/platform-data'`。`@reelclone/common` 现为纯底层库（仅依赖 `@nestjs/*` + `uuid` + `dayjs`）。
...
已实现。`@reelclone/capability` 包统一了 8 种生成类型的 Provider 路由、积分定价（FixedPoints + MatrixPoints）、参数校验、模型/UI 配置和 work type 映射。后端 3 处硬编码（shared.ts / points-calculator / create.dto）和前端 7 个页面已迁移到 registry。44 个单元测试全通过（含 12 个评审修复新增测试）。

**评审修复（2026-08-02）：** 评审发现 9 个问题（P0×3 + P1×5 + P2×1），已全部修复：

| 问题              | 修复                                                                                                      |
| ----------------- | --------------------------------------------------------------------------------------------------------- |
| P0-1 多实例不一致 | 后端通过 NestJS DI (`@Inject(CAPABILITY_REGISTRY)`) 统一单实例                                            |
| P0-2 静默降级     | `calculatePoints()` fail-fast；`strict` 模式 `getProvider/getWorkType/getTemporalWorkType` 未知类型抛异常 |
| P0-3 类型无关联   | `TemporalWorkTypeName` 硬编码字符串字面量 + `validate()` 运行时校验 temporalWorkType 合法性               |
| P1-4 启动校验     | 构造时自动校验枚举覆盖、matrix 一致性（base↔resolutions, multiplier↔durations）                           |
| P1-5 MOCK 语义    | `TEXT_GENERATE`/`IMAGE_GENERATE` 的 temporalWorkType 行添加 MOCK 注释                                     |
| P1-6 DTO 分层     | `create-generation.dto.ts` 添加校验分层注释                                                               |
| P1-7 后端统一     | `workbench.module.ts` 导入 `CapabilityModule`，所有 handler 通过 DI 注入 registry                         |
| P1-8 validate     | `CapabilityRegistry` 构造函数新增 `options?: { strict?: boolean }`，默认 false                            |
| P2-10 前端优化    | `capabilities.ts` 新增预计算 `VIDEO_POINTS_TABLES` 查找表                                                 |
...
> **已完成（2026-08-03）**：AES-256-GCM 应用层加密 — `libs/common/src/crypto/secret-encryption.ts` 提供 `encryptSecret()`/`decryptSecret()`，`enc:v1:` 前缀约定实现向后兼容（无前缀明文透传）。`ConfigStoreService` 的 `get()`/`set()` 集成加解密，DB 和 Redis 均存储密文。12 个单元测试覆盖往返、幂等、兼容性、容错。`scripts/encrypt-system-config.ts` 一次性迁移脚本加密存量明文值。部署需设置 `SECRET_ENCRYPTION_KEY`（64 字符 hex = 32 字节 AES-256 密钥）。

#### ~~P1-6 配置和连接拓扑重复且漂移~~ ✅

> **已完成（2026-08-03）**：创建 `@reelclone/common` 共享模块三件套 — `ServiceConfigModule.forRoot()`（统一 ConfigModule.load 为 `[configuration, jwtConfig]` + 可选 `extraLoad`）、`ServiceJwtModule.forRoot()`（统一 JwtModule.registerAsync 三层 fallback）、`RedisBridgeModule.forRoot()`（统一 `OBS_REDIS_CLIENT → REDIS_CLIENT` 桥接）。11 个微服务的 `app.module.ts` 已全部迁移，消除 ~600 行重复配置代码。新增 3 个单元测试文件覆盖模块工厂行为。
...
> **已完成** `26d6d1b` — 修复方案：从 main 库 `CreditOperation.idempotencyKey` 提取已发放序号集合，枚举 `[1..useCount]` 中缺失序号逐条补发，替代 `COUNT(*)` 推导。
>
> 变更文件：`billing.service.ts` (+`getRewardOrdinalsByTemplateId`)、`billing.controller.ts` (+reward-ordinals endpoint)、`billing.client.ts` (+`getRewardOrdinals`)、`reward-reconciliation.service.ts` (重写 reconcile 逻辑)
>
> 回归测试：`billing.service.spec.ts` (+4)、`billing.client.spec.ts` (+5)、`reward-reconciliation.service.spec.ts` (新建，9 个间隙场景用例)
...
| ~~benchmark freeze/settle/release 独立 key + durable reservation~~ |     P0 |    M | ✅ 94ae726 V2 CreditReservation（独立幂等键 + reservationMode=true）；68dfb27 端到端验证测试 + 灰度方案 |
...
| CI 执行真实 Nx/Taro/Vite build 和正确 Docker matrix                |     P0 |    M | ✅ 15b2343 Docker matrix 路径修正 + 小程序构建门禁                                                      |
...
| ~~外部 URL/asset ownership policy~~                                |     P0 |    M | ✅ 868d8c2 ossKey 前缀校验 + validateRedirect 已实现                                                    |
...
4. ~~capability registry 统一 quote、validation、UI 和 Provider mapping。~~ ✅ E1 已完成：创建 `@reelclone/capability` 包（libs/capability/），统一 5 个维度的硬编码配置。GenerationType 枚举从 DTO/AI/Temporal 3 套合并为单一真相源；积分定价矩阵（FixedPoints + MatrixPoints）覆盖 8 种生成类型；参数校验/模型选项/UI 配置/provider 路由/work type 映射全部声明式。后端：shared.ts 从 switch/case 改为 registry.getWorkType()；points-calculator 从内联表改为 registry.calculatePoints()；create-generation.dto 从本地枚举改为 import re-export。前端：新建 utils/capabilities.ts 薄封装层，7 个页面（video-text/image-first/image-first-last/video-edit/video-extend/text/image）从硬编码常量迁移到 registry。31 个单元测试全通过。实现文件：`libs/capability/`(新包)、`libs/capability/src/capability.registry.ts`(31 方法)、`libs/capability/src/capability.default.ts`(8 类型种子数据)、`apps/workbench-service/src/workbench/generation/shared.ts`(getWorkType/getProvider)、`apps/workbench-service/src/workbench/points-calculator.util.ts`(calculatePoints/isVideoType)、`apps/workbench-service/src/workbench/dto/create-generation.dto.ts`(import re-export)、`apps/miniprogram/src/utils/capabilities.ts`(前端封装)、7 个 miniprogram 页面。
...
3. ~~将 872 行 `GenerationService` 按 use case 拆为 create/query/cancel/retry/finalize/reconcile handlers。~~ ✅ D3 已完成：GenerationService 从 872 行缩减为 132 行薄 Facade，委托 GenerationCreateHandler、GenerationCancelHandler、GenerationRetryHandler 三个 handler 执行业务逻辑。findAll/findOne 直接在 Facade 中委托共享函数。finalize/reconcile 逻辑已在 Phase 0 的 Temporal workflow（C5 ReconcilerWorkflow）中实现，无需在 NestJS 服务层重复。实现文件：`apps/workbench-service/src/workbench/generation.service.ts`(Facade 132 行)、`apps/workbench-service/src/workbench/generation/create.handler.ts`、`generation/cancel.handler.ts`、`generation/retry.handler.ts`、`generation/shared.ts`。
4. ~~统一 internal HTTP client：超时、retry classification、idempotency、trace、typed response。~~ ✅ D4 已完成：创建 `@reelclone/http-client` 包（libs/http-client/），InternalHttpClient 类（216 行）提供超时（默认 10s per-request）、重试（网络错误 + 5xx 指数退避，4xx 不重试）、熔断器（CLOSED→OPEN→HALF_OPEN）、幂等（自动注入 x-idempotency-key）、Trace（自动注入 x-request-id）、API response 自动解包和异常映射。含单元测试。实现文件：`libs/http-client/src/http-client.ts`、`circuit-breaker.ts`、`http-client.spec.ts`、`circuit-breaker.spec.ts`、`index.ts`。
5. ~~每个 service 只初始化需要的 DB/Redis/adapter。~~ ✅ D5 已完成：`DatabaseModule.forRoot()` 支持 `connections` 参数按需选择。所有 11 个服务（auth/user/asset/benchmark/template/billing/workbench/notification/order/admin/media-worker）均按实际需要选择性初始化 DB 连接，无服务仍使用全部 4 个连接。实现文件：`libs/database/src/modules/database.module.ts`（forRoot 接受 selective connections）、各服务 `app.module.ts`。

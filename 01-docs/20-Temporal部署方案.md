...
3. 云托管的「仅支持 HTTP 协议」限制针对**云托管入站网关**（外部请求→云托管）；**容器到腾讯云资源的 VPC 内网调用不受此限**——官方文档将内网调用定义为"通过私有网络设置打通腾讯云资源、使用资源 VPC 内网 IP 调用"（以 Redis 为例），同一 VPC 内数据面为普通 TCP 网络，Temporal gRPC（HTTP/2 over TCP 7233）经内网连接正常。

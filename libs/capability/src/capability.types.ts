...
/**
 * Temporal WorkType 枚举的值类型（硬编码字符串字面量联合类型）。
 *
 * 与 Temporal 层的 WorkType 枚举保持手动同步。
 * 构造 CapabilityRegistry 时，validate() 会做运行时二次校验，
 * 确保 DEFAULT_CAPABILITIES 中的 temporalWorkType 值都在此集合内。
 */
